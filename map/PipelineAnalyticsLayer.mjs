...
export class PipelineAnalyticsLayer {
 constructor(map,onSelect){
  ...
  map.addLayer({id:'pipeline-analytics-missing',type:'line',source:'pipeline-analytics',layout:{visibility:'none'},paint:{'line-color':'#999','line-width':3,'line-dasharray':[2,2]}});
 ...
 hit(point){return this.enabled?this.map.queryRenderedFeatures(point,{layers:['pipeline-analytics-lines','pipeline-analytics-missing']})[0]:null;}
 update({enabled,filtered,selected,color,unit,group}){
  this.enabled=enabled;this.selected=selected;
  for(const id of ['pipeline-analytics-lines','pipeline-analytics-missing','pipeline-analytics-highlight'])this.map.setLayoutProperty(id,'visibility',enabled?'visible':'none');
  ...
  this.map.setFilter('pipeline-analytics-lines',['all',['in',['get','id'],['literal',ids]],['!',missing]]);
  this.map.setFilter('pipeline-analytics-missing',['all',['in',['get','id'],['literal',ids]],missing]);
